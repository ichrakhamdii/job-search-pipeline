...
- **A second LLM pass (the "judge") on top of the embedding score.** Embeddings alone still produce false positives, a posting can score high because the title matches while the actual role is completely different. The judge re-reads each shortlisted posting against my actual background and drops what doesn't really fit, instead of trusting one similarity number.
